...
# financial donations from generous benefactors. John has a list of the first
# n donations: [14, 30, 5, 7, 9, 11, 15] He wants to know how much the next
# benefactor should give to the association so that the average of the first
# n + 1 donations should reach an average of 30. After doing the math he found
# 149. He thinks that he could have made a mistake.
...
def new_avg(arr, newavg):
    ...
    new_donation = ceil(newavg * (len(arr) + 1) - sum(arr))
    assert new_donation > 0
    return new_donation
...
# expect_error("Error expected", lambda : new_avg ([14, 30, 5, 7, 9, 11, 15], 2))
